...
int mergesort(Data_Set **data_set, int column_no_for_sort, int start, int end){
    ...
    if(start < end)
    {
        ...
        ret = merge(data_set, column_no_for_sort, start, mid, end);    		
        if (ret < 0){
        	return (-1);
	}
    }
...
int merge(Data_Set **data_set, int column_no_for_sort, int start, int mid, int end){
    ...
    for (i = tmp_index; i < end - start; i++){
    	while(p <= mid && q <= end){
    		...
    		*(str2 + strlen(data_set[q]->field_ptr[column_no_for_sort])) = 0;
    		...
    		trimString(str2);
                ...
                //int data type
		if (data_set[p]->type[column_no_for_sort] == 1 && data_set[q]->type[column_no_for_sort] == 1 ){	
		...
		// float type
		else if (data_set[p]->type[column_no_for_sort] == 2 && data_set[q]->type[column_no_for_sort] == 2){	
